...
namespace cAlgo
{
    ...
    public class MC_EURCHF : Indicator
    {
        [Output("Result")]
        ...
        public override void Calculate(int index)
        {
            ...
            {
                Result[index] = (_symbolbigSeries.Close[index] - 1 / _symbolsmallSeries.Close[SmallIndex]) / 0.0001;
            }
            if (Symbol.Code == SmallSymbol)
            {
                Result[index] = (_symbolbigSeries.Close[BigIndex] - 1 / _symbolsmallSeries.Close[index]) / 0.0001;
            }
            double sum = 0.0;
