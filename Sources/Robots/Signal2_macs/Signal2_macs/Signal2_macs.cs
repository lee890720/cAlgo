...
namespace cAlgo.Robots
{
    ...
    public class Signal2_macs : Robot
    {
        private int _id;
        ...
        protected override void OnTimer()
        {
            if (double.IsNaN(_result.Result.LastValue) || double.IsInfinity(_result.Result.LastValue) || double.IsNaN(_average.Result.LastValue) || double.IsInfinity(_average.Result.LastValue))
                return;
            var cr = _result.Result.LastValue / Symbol.PipSize;
            var ca = _average.Result.LastValue / Symbol.PipSize;
            var sr = cr - ca;
            double sum = 0.0;
            for (int i = _averagePeriods; i > 0; i--)
            {
                sum += (_result.Result.Last(i) - _average.Result.Last(i)) / Symbol.PipSize;
            }
