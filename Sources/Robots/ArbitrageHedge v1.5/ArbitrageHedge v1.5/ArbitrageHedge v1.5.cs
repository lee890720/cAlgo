...
namespace cAlgo
{
    ...
    public class ArbitrageHedge : Robot
    {
        ...
        private OrderParams initBuyOne, initBuyTwo, initSellOne, initSellTwo;
        ...
        protected override void OnStart()
        {
            ...
            initBuyOne = new OrderParams(TradeType.Buy, Symbol, Init_Volume, MaAbove, null, null, slippage, null, null, new System.Collections.Generic.List<double> 
            {
                            });
            initBuyTwo = new OrderParams(TradeType.Buy, OtherSymbol, Init_Volume, MaBelow, null, null, slippage, null, null, new System.Collections.Generic.List<double> 
            {
                            });
            initSellOne = new OrderParams(TradeType.Sell, Symbol, Init_Volume, MaBelow, null, null, slippage, null, null, new System.Collections.Generic.List<double> 
            {
                            });
            initSellTwo = new OrderParams(TradeType.Sell, OtherSymbol, Init_Volume, MaAbove, null, null, slippage, null, null, new System.Collections.Generic.List<double> 
            {
                            });
        }
        ...
        protected override void OnTick()
        {
            ...
            initBuyOne.Comment = subtract;
            initBuyTwo.Comment = subtract;
            initSellOne.Comment = subtract;
            initSellTwo.Comment = subtract;

            if (opensignal() == "BuyOneAndSellTwo")
            {
                this.executeOrder(initBuyOne);
                this.executeOrder(initSellTwo);
            }
            if (opensignal() == "SellOneAndBuyTwo")
            {
                this.executeOrder(initSellOne);
                this.executeOrder(initBuyTwo);
            }
            if (closesignal() == "CloseMaAbove")
            {
                this.closeAllLabel(MaAbove);
            }
            if (closesignal() == "CloseMaBelow")
            {
        ...
        private string opensignal()
        {
            ...
            if (positions.Count == 0)
            {
                if (result > average + Distance)
                    sig = "BuyOneAndSellTwo";
                if (result < average - Distance)
                    sig = "SellOneAndBuyTwo";
            }
            var now = DateTime.UtcNow;
            if (MaAbovePos.Count != 0)
                if (DateTime.Compare(MaAbovePos[0].EntryTime.AddHours(1), now) < 0)
                    if (result > average + aboveaverage)
                        sig = "BuyOneAndSellTwo";
            if (MaBelowPos.Count != 0)
                if (DateTime.Compare(MaBelowPos[0].EntryTime.AddHours(1), now) < 0)
