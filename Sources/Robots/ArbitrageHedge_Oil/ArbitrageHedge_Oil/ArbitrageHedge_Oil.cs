...
namespace cAlgo
{
    ...
    public class ArbitrageHedge_Oil : Robot
    {
        ...
        protected override void OnTick()
        {
            ...
            #region Close_Break
            if (GetBreakCount(AboveLabel) != 0)
            {
                if (this.TotalProfits(AboveLabel) / (_firstsymbol.PipValue * Init_Volume) > 25 * GetBreakCount(AboveLabel))
                {
                    this.closeAllLabel(AboveLabel);
                }
            }
            if (GetBreakCount(BelowLabel) != 0)
            {
                if (this.TotalProfits(BelowLabel) / (_firstsymbol.PipValue * Init_Volume) > 25 * GetBreakCount(BelowLabel))
                {
                    this.closeAllLabel(BelowLabel);
                }
            }
