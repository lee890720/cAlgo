...
namespace cAlgo
{
    ...
    public class Bollinger_Deviations_ATR : Robot
    {
        #region Parameter
        ...
        bool IsBuy, IsSell, IsB_In, IsB_N, IsB_De, IsS_In, IsS_N, IsS_De;
        double atr_in;
        #endregion
        ...
        protected override void OnBar()
        {
            ...
            if (Symbol.Ask < bottom_decrease)
                IsB_De = true;
            if (IsB_In && Symbol.Ask > bottom)
                IsBuy = true;
            if (!IsB_In && IsB_N && Symbol.Ask > bottom_decrease)
                IsBuy = true;
            if (!IsB_In && !IsB_N && IsB_De && Symbol.Ask > bottom_decrease)
                IsBuy = true;
            ...
            if (Symbol.Bid > top_decrease)
                IsS_De = true;
            if (IsS_In && Symbol.Bid < top)
                IsSell = true;
            if (!IsS_In && IsS_N && Symbol.Bid < top_decrease)
                IsSell = true;
            if (!IsS_In && !IsS_N && IsS_De && Symbol.Bid < top_decrease)
                IsSell = true;
