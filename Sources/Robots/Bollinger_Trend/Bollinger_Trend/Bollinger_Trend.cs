...
namespace cAlgo
{
    ...
    public class Bollinger_Trend : Robot
    {
        #region Parameter
        ...
        private void PositionsOnOpened(PositionOpenedEventArgs args)
        {
            ...
            if (args.Position.TradeType == TradeType.Buy)
            {
                if (b_buypositions.Length == 1)
                    foreach (var position in b_buypositions)
                        ModifyPosition(args.Position, main, null);
                if (b_buypositions.Length > 1)
                    foreach (var position in b_buypositions)
                        ModifyPosition(args.Position, AveragePrice(b_buypositions), null);
            }
            if (args.Position.TradeType == TradeType.Sell)
            {
                ...
                if (b_sellpositions.Length > 1)
                    foreach (var position in b_sellpositions)
                        ModifyPosition(args.Position, AveragePrice(b_sellpositions), null);
            }
        }
        ...
        protected override void OnBar()
        {
            ...
            {
                //Open b_selllabel
                if (Symbol.Ask < bottom && !bollingerBands.Top.IsFalling())
                {
                    if (b_sellpositions.Length == 0)
                    {
                ...
                //Open b_buylabel
                if (Symbol.Bid > top && !bollingerBands.Bottom.IsRising())
                {
                    if (b_buypositions.Length == 0)
                    {
